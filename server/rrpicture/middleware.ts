...
const isPictureAndTypeValid = async (req: Request, res: Response, next: NextFunction) => {
  if (!req.body.picture.trim()) {
    res.status(400).json({
      error: 'The picture must be at least one character long.'
    });
    return;
  }
  ...
  if (!req.body.pictureType.trim()) {
    res.status(400).json({
      error: 'The picture type must be at least one character long.'
    });
    return;
  }
  ...
  if (!RRPictureTypeValues.has(req.body.pictureType)) {
    res.status(400).json({
      error: `The picture type "${req.body.pictureType}" is not valid.`
    });
    return;
  }
...
const isUsersPicture = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!rrpicture) {
    res.status(404).json({
      error: `The profile picture with id ${req.params.rrpictureId} does not exist for the user.`,
    });
    return;
  }
  next();
